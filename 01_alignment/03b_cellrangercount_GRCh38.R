...
'--id='%&% batches[b] %&%'-' %&% conditions[c] %&%'_GRCh38' %&%
...
' --transcriptome=/project/lbarreiro/USERS/daniel/asthma_project/alignment/GRCh38 --jobmode=slurm'

# create sbatch file
cat(sbatch_topper%&%'\n\n'%&%command_line,
file=batches[b]%&%'_'%&% conditions[c]%&%'_cellrangercount_GRCh38.sbatch', append=F)

#submit job
system('sbatch '%&%batches[b]%&%'_'%&%conditions[c]%&%'_cellrangercount_GRCh38.sbatch')
}
}
