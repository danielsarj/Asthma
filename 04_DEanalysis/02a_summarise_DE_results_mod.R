...
for (i in 1:length(conditions)){
  for (ctype in cells_seurat){
    ...
    results <- fread('NI_'%&%conditions[i]%&%'_limma_'%&%ctype%&%'_results_mod.txt') %>% 
    ...
    ggplot(results) + geom_point(aes(logFC, -log10(adj.P.Val)), size=0.5, alpha=0.5) +
      ...
      geom_text_repel(aes(logFC, -log10(adj.P.Val), label=ifelse(adj.P.Val<0.00001,V1, '')), 
                      colour='red', size=3)
    ggsave('NI_'%&%conditions[i]%&%'_limma_'%&%ctype%&%'_volcanoplot_mod.pdf', height=3, width=4)
...
ggsave('NI_IVAxRV_limma_facetgrid_volcanoplot_mod.pdf', height=5, width=8)
...
ggsave('IVAxRV_limma_logFCcorrelation_scatterplot_mod.pdf', height=5, width=6)
...
ggsave('NI_IVAxRV_NumOfDEgenes_barplot_mod.pdf', height=4, width=6)
...
for (ctype in cells_seurat){
  ...
  pdf('NI_IVAxRV_SharedDEgenes_'%&%ctype%&%'_upsetplot_mod.pdf', height=3, width=4, onefile=F)
  plot.new() 
