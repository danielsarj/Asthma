...
for (cond in conditions){
  ...
  for (ctype in celltypes){
    ...
    # subset mdata 
    sub_mdata <- sub_mdata %>% select(IDs, condition, celltype, IFNa_response1, IFNy_response1, asthma)
...
# now indv level 
summary_scores <- ifn.scores %>% group_by(IDs, condition, celltype, asthma) %>% 
  summarise(IFNa_response=mean(IFNa_response1), IFNy_response=mean(IFNy_response1)) %>% ungroup()

summary_scores %>% ggplot(., aes(x=condition, y=IFNa_response, fill=asthma)) + geom_boxplot() + 
  facet_wrap(~celltype, scale='free') + theme_bw() 
summary_scores %>% ggplot(., aes(x=condition, y=IFNy_response, fill=asthma)) + geom_boxplot() + 
  facet_wrap(~celltype, scale='free') + theme_bw()   
